...
def main():
    ...
    hdr = f"{'Condition':<20}  " + '  '.join(f'{k:<30}' for k in metric_keys + ['rvel'])
    print(hdr)
    print('-' * (22 + 32 * (len(metric_keys) + 1)))

    agg_rows = []
    for cond_name in CONDITIONS:
        song_means = {k: [] for k in metric_keys + ['rvel']}
        song_labels = []
        ...
        for song in SONGS:
            ...
            rv_vals = [rvel(e['match_rate'], e['velocity_mae'], VBAR[song])
                       for e in entries
                       if e.get('match_rate') is not None and e.get('velocity_mae') is not None]
            if rv_vals:
                song_means['rvel'].append(np.mean(rv_vals))

        cells = []
        row = {'Condition': cond_name}
        coverage = f'({",".join(song_labels)})' if song_labels else ''
        for k in metric_keys + ['rvel']:
    ...
    for song in SONGS:
        ...
        for cond_name in CONDITIONS:
            ...
            rv_vals = [rvel(e['match_rate'], e['velocity_mae'], VBAR[song])
                       for e in entries
                       if e.get('match_rate') is not None and e.get('velocity_mae') is not None]
            rv_mean, rv_hw = ci95(rv_vals)
            if rv_mean is None:
                cells.append(f'{"--":<28}')
            elif rv_hw is None:
                cells.append(f'{rv_mean:.3f}{tag:<23}')
            else:
                cells.append(f'{rv_mean:.3f} ± {rv_hw:.3f}{tag:<15}')
            print(f'{cond_name:<20}  ' + '  '.join(cells))

    # ── Save CSV ──────────────────────────────────────────────────────────────
    csv_path = out_dir / 'aggregated.csv'
    fieldnames = ['Condition'] + [f for k in metric_keys + ['rvel'] for f in (k, f'{k}_ci')]
